...
export function usePrimaryCurrency() {
  ...
  // Listen to cross-component and cross-tab currency changes
  useEffect(() => {
  ...
  // 1. localCurrency (if user explicitly selected during session or received event)
  // 2. userSettings?.primaryCurrency (from Convex database)
  // 3. localStorage (persisted browser preference)
  // 4. Default "IDR"
