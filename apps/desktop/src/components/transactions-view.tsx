  ...
  return (
    <div className="space-y-4 max-w-5xl mx-auto pb-12">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">Log expenses, income, and transfers.</p>
      </div>
      ...
      <div className="rounded-lg border border-border bg-background p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            ...
            <p className="text-[11px] text-muted-foreground">Net {formatCurrencyAmount(totals.net, primaryCurrency)}</p>
          </div>
          <Button variant="outline" size="icon-sm" onClick={() => setMonth(shiftMonth(month, 1))}>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/20 p-3 text-center">
            ...
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium mt-0.5">Income</div>
          </div>
          <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-3 text-center">
            ...
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium mt-0.5">Expenses</div>
          </div>
        </div>
      </div>
      ...
      {transactions === undefined ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-12 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground/60">{search ? "Try a different search" : "Add your first transaction"}</p>
        </div>
      ) : (
                    ...
                    <div key={t._id} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <div className="shrink-0 text-right">
                        ...
                        {t.currency !== primaryCurrency && (
                          <div className="text-[10px] text-muted-foreground">{t.amount} {t.currency}</div>
                        )}
                      </div>
                      <div className="flex shrink-0 items-center">
                        <Button variant="ghost" size="icon-sm" onClick={() => openEdit(t)} className="size-7 cursor-pointer">
                          <Pencil className="size-3.5" />
                        </Button>
                        <Button variant="ghost" size="icon-sm" onClick={() => setDeleting(t)} className="size-7 hover:text-destructive cursor-pointer">
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-sm rounded-lg p-5">
          <DialogHeader className="space-y-1 text-left">
            ...
            <DialogDescription className="text-xs text-muted-foreground">Update amount, date, category, or note.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs font-medium">Amount</label>
                <Input type="number" step="0.01" value={editAmount} onChange={(e) => setEditAmount(e.target.value)} className="text-xs" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium">Date</label>
                <Input type="date" value={editDate} onChange={(e) => setEditDate(e.target.value)} className="text-xs" />
              </div>
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium">Category</label>
              <select value={editCategory} onChange={(e) => setEditCategory(e.target.value)} className="flex h-9 w-full rounded-lg border border-border bg-background px-2.5 text-xs">
            ...
            <div className="space-y-1">
              ...
              <Input value={editNote} onChange={(e) => setEditNote(e.target.value)} className="text-xs" placeholder="Optional note" />
            </div>
          </div>
          <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => setEditing(null)} className="cursor-pointer">Cancel</Button>
            <Button size="sm" onClick={handleSaveEdit} disabled={isSaving} className="cursor-pointer">{isSaving ? "Saving..." : "Save Changes"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
        <DialogContent className="max-w-sm rounded-lg p-5">
          <DialogHeader className="space-y-1 text-left">
            ...
            <DialogDescription className="text-xs text-muted-foreground">This will permanently remove this transaction.</DialogDescription>
          </DialogHeader>
          <div className="flex gap-2 justify-end mt-3">
            <Button variant="outline" size="sm" onClick={() => setDeleting(null)} className="cursor-pointer">Cancel</Button>
            <Button variant="destructive" size="sm" onClick={handleDelete} disabled={isSaving} className="cursor-pointer">{isSaving ? "Deleting..." : "Delete"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
