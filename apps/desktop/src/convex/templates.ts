...
const defaultTemplates = [
  ...
  { name: "YouTube Music", icon: "Youtube", color: "#FF0000", category: "music", defaultPrice: 10.99, defaultCurrency: "USD", cancelUrl: "https://www.youtube.com/paid_memberships" },
  ...
  { name: "Adobe Creative Cloud", icon: "Palette", color: "#FF0000", category: "design", defaultPrice: 54.99, defaultCurrency: "USD", cancelUrl: "https://account.adobe.com/plans" },
  { name: "Canva Pro", icon: "Paintbrush", color: "#00C4CC", category: "design", defaultPrice: 12.99, defaultCurrency: "USD", cancelUrl: "https://www.canva.com/settings/billing-and-teams" },
  { name: "Figma", icon: "PenTool", color: "#F24E1E", category: "design", defaultPrice: 12.0, defaultCurrency: "USD", cancelUrl: "https://www.figma.com/settings" },
]
...
export const seed = internalMutation({
  args: {},
  handler: async (ctx) => {
    const count = await ctx.db.query("templates").collect()
...
export const seedManual = mutation({
  args: {},
  handler: async (ctx) => {
    const existing = await ctx.db.query("templates").collect()
    if (existing.length > 0) return { message: "Templates already seeded", count: existing.length }
    for (const t of defaultTemplates) {
      await ctx.db.insert("templates", t)
    }
    return { message: "Seeded templates", count: defaultTemplates.length }
  },
})
